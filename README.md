# CZSC 结构监控项目 · 5000 根实时数据版 v0.10.7

## v0.10.7 首段边界校验 + 有缺口端点迁移合并版

本版本合并了两项互不替代的线段修复：

1. **有限窗口首段边界修复**：不再采用“第一个能成段的起点”。程序会扫描
   所有可行起点，排除段内已经出现更低起点/更高终点（或对称情况）的首段
   候选，并选择终点最早的合法完整候选。候选之前的笔明确进入
   `unresolved_segment_prefix_strokes`。
2. **有缺口等待期端点迁移修复**：主特征分型存在缺口时，反向特征序列等待
   期间继续跟踪同类笔端点；出现更高顶或更低底时迁移端点并从新端点重启
   反向确认，不再锁死旧端点。

两项状态同时生效：首段通过多起点与段内极值校验后，后续每条有缺口线段仍
执行端点迁移。独立参考实现也改为完整事件回放：先按实际检测时间选择主特征
候选，再独立执行缺口端点迁移。

同一批 5000 根确定性 5m 数据的合并结果：
...
需要说明：首段段内极值校验能排除当前窗口内可证明错误的候选，但不能凭空
