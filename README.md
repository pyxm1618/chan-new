# CZSC 结构监控项目 · 5000 根实时数据版 v0.10.9

## v0.10.9 缺口等待中的主确认 / 反向确认竞争修复

本版修复了 v0.10.7 有缺口状态机仍然存在的第二层问题。

旧版已经允许缺口候选端点随更高顶或更低底迁移，但进入缺口等待状态后，
只继续检查原始同类笔极值和反向特征分型，**没有继续消费主特征序列中随后
已经确认的无缺口分型**。因此可能发生：

```text
端点 21：有缺口，位置 23 确认，进入等待
端点 25：无缺口，位置 27 已确认
端点 31：无缺口，位置 33 已确认
```

正确结果应在位置 27 直接按 `NO_GAP` 确认 `18 → 25`；旧版却继续迁移并
最终输出 `18 → 31 / GAP_REVERSE_FRACTAL / 36`。

现在缺口等待被实现为两个确认事件的持续竞争：

```text
有缺口主分型进入等待
├─ 后续有效无缺口主分型先确认 → 立即 NO_GAP 结束
├─ 新同类极值出现             → 迁移端点并重启反向检测器
└─ 反向特征分型先确认         → GAP_REVERSE_FRACTAL 结束
```

同一笔位置同时出现事件时，优先级为：有效无缺口主分型、端点迁移、反向
确认。这样不会因为已经进入缺口分支而忽略更早完成的第一种确认。

v0.10.7 的两项修复继续保留：

1. 有限窗口首段多起点与段内极值边界校验；
2. 缺口等待期间更高顶 / 更低底的端点迁移。

同一批 5000 根确定性 5m 数据的新结果：
...
新增硬性保护 `GAP_CONFIRMATION_SKIPPED_EARLIER_NO_GAP_PRIMARY`：任何按
缺口反向分型确认的线段，在其确认时间之前都不得存在一个尚未被更极端端点
淘汰、且已经确认的无缺口主分型。

需要说明：首段段内极值校验仍只能排除当前窗口内可证明错误的候选，不能凭空
恢复交易所历史起点之前的数据。完整历史追加存储与持久化分析状态仍是彻底
消除左边界不确定性的长期架构方案。
...
笔状态机允许最新一笔在后续 K 线创新高或创新低时撤销。为避免已确认线段依赖这条可回撤笔，正式分析现在要求：
