...
def main() -> None:
    ...
    for offset in range(min(40, max(0, len(result.strokes) - 3))):
        truncated = detect_segments(result.strokes[offset:])
    ...
    bug_regression = {
        ...
        "v0.10.7_merged_boundary_and_gap_fixes": {
            "confirmed_strokes": len(result.strokes),
            "confirmed_segments": len(result.segments),
            "feature_elements": len(result.feature_elements),
            "feature_fractals": len(result.feature_fractals),
    ...
    trace_start = 303
    active_position = 308
    active_value = result.strokes[active_position].fx_a.value
    trace_rows.append({
        "event": "gap_origin",
        "stroke_position": active_position,
        "endpoint_time": result.strokes[active_position].fx_a.dt,
        "endpoint_price": active_value,
    })
    for position in range(active_position + 1, 327):
        stroke = result.strokes[position]
        if stroke.direction.value != "down":
            continue
        endpoint = stroke.fx_a
        if endpoint.value > active_value:
            active_position = position
            active_value = endpoint.value
            trace_rows.append({
                "event": "replace",
                "stroke_position": position,
                "endpoint_time": endpoint.dt,
                "endpoint_price": endpoint.value,
            })
    trace_rows.append({
        "event": "reverse_confirm",
        "stroke_position": 337,
        "endpoint_time": result.strokes[337].end_dt,
        "endpoint_price": active_value,
    })
