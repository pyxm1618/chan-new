...
def main() -> None:
    ...
    for remaining in (1, 2):
        start = len(demo.strokes) - remaining
        tail = detect_segments_from_anchor(
            demo.strokes,
            start_position=start,
            mode=demo.segment_mode,
            exclude_last_stroke_confirmation=False,
        )
        issues = validate_feature_sequence_coverage(
            tail.feature_elements,
            demo.strokes,
            scan_start_position=start,
        )
        if issues:
            short_tail_failures.append({
                "remaining": remaining,
                "issues": [
                    {"code": item.code, "message": item.message}
                    for item in issues
    ...
    output = {
        ...
        "short_tail_failure_count": len(short_tail_failures),
        "short_tail_failures": short_tail_failures,
    }
    print(json.dumps(output, ensure_ascii=False, indent=2))

    if (
        real_issues
        or anchored_coverage_issues
        or prefix_failures
        or demo_issues
        or short_tail_failures
    ):
        raise SystemExit(1)
