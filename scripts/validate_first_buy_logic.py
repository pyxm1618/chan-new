...
def _with_internal_strokes(
    ...
    strokes = tuple(
        _stroke(a, b, 1000 + segment.index * 100 + i, durations[i])
        for i, (a, b) in enumerate(zip(points, points[1:]))
    )
...
def _synthetic_validation(v: Validation, random_cases: int) -> dict[str, object]:
    ...
    durations = [5, 5, 5, 5, 20, 5, 5, 5, 5, 5, 2]
    ...
    v.require(not validate_trading_points(result.points, segments, zones, raw_bars=bars), "deterministic_validator")
    ...
    forged_codes = {
        x.code for x in validate_trading_points((forged,), segments, zones, raw_bars=bars)
    }
    ...
    weakened_codes = {
        x.code for x in validate_trading_points(
            (formal_b1,), weakened, weakened_zones, raw_bars=weakened_bars
        )
    }
...
def _dataset_validation(v: Validation, *, demo_count: int, real_count: int) -> dict[str, object]:
    ...
    for position, bar in enumerate(demo, start=1):
        ...
        v.require(
            not validate_trading_points(
                result.points, state.segments, zones, raw_bars=demo[:position]
            ),
    ...
    v.require(
        not validate_trading_points(
            real.trading_points, real.segments, real.segment_central_zones, raw_bars=real.raw_bars
        ),
        "real_validator",
    )
