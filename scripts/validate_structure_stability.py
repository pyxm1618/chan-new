...
def validate(count: int) -> dict[str, object]:
    # DEMO 从生成器真实起点开始；该脚本专门验证右边界提交账本。
    ...
    for position, bar in enumerate(
        ...
        if len(segments) != len(previous_segments):
            formal_segment_zones, _ = _detect_formal_segment_central_zones(
                state.segments,
                left_boundary_anchored=state.left_boundary_anchored,
            )
            current_segment_zones = {
                _segment_zone_seed(item): item
                for item in formal_segment_zones.zones
            }
    ...
    return {
        ...
        "final_resolved_strokes": len(state.resolved_stable_strokes),
        "final_unresolved_prefix_strokes": len(state.unresolved_prefix_strokes),
        "final_provisional_strokes": len(state.provisional_strokes),
        "final_unresolved_prefix_segments": len(state.unresolved_prefix_segments),
