...
def strokes_frame(result: AnalysisResult) -> pd.DataFrame:
    resolved_start = len(result.stable_strokes) - len(result.resolved_strokes)
    return pd.DataFrame(
        [
            {
                "笔序号": stroke.index,
                "状态": (
                    "LEFT_UNRESOLVED"
                    if (
                        not result.left_boundary_resolved
                        or stroke.index < resolved_start
                    )
                    else (
                        "STABLE"
        ...
        rows.append(
            {
                ...
                "正式提交时间": evidence.committed_at if evidence else None,
                "正式提交原始K位置": (
                    evidence.committed_at_bar_position if evidence else None
                ),
                "价差力度": segment.power,
                "涨跌幅": segment.change,
        ...
        [
            {
                ...
                "确认证据可用时间快照": item.confirmation_available_at,
                "确认证据笔指纹": item.confirmation_stroke_fingerprint,
                "正式提交时间": item.committed_at,
                "正式提交原始K位置": item.committed_at_bar_position,
                "线段品种绑定": item.segment_symbol,
                "线段周期绑定": item.segment_interval,
                "线段结构指纹": item.segment_fingerprint,
        ...
        [
            {
                ...
                "GG最高": zone.gg,
                "DD最低": zone.dd,
                "趋势GG（不含最终离开）": zone.trend_gg,
                "趋势DD（不含最终离开）": zone.trend_dd,
                "有效": zone.is_valid,
        ...
        [
            {
                ...
                "比较单元b": item.entry_unit_index,
                "离开单元c": item.exit_unit_index,
                "b方向MACD面积": item.entry_macd_area,
                "c方向MACD面积": item.exit_macd_area,
                "进入价格力度": item.entry_power,
                "离开价格力度": item.exit_power,
                "创新极值": item.price_extreme,
                "MACD背驰": item.macd_divergence,
                "MACD状态精确": item.macd_state_exact,
                "严格GG/DD趋势": item.strict_trend,
                "c内次级别三类点": item.sublevel_third_point,
                "c内次级别中枢数": item.sublevel_zone_count,
                "最终有效": item.is_valid,
                "b时间": f"{item.entry_start_dt.isoformat()} ~ {item.entry_end_dt.isoformat()}",
                "c时间": f"{item.exit_start_dt.isoformat()} ~ {item.exit_end_dt.isoformat()}",
            }
...
def _add_result_provisional_strokes(
    ...
    if not result.left_boundary_resolved:
        unresolved_values = tuple(
            ProvisionalLine(
                ...
                reason="窗口左侧缺少真实历史起点或持久化锚点，全部笔仅作候选",
                source_indexes=(item.index,),
            )
            for item in result.strokes
        )
        tail_values: tuple[ProvisionalLine, ...] = ()
    else:
        unresolved_count = len(result.stable_strokes) - len(result.resolved_strokes)
        unresolved_values = tuple(
            ProvisionalLine(
                structure="stroke",
                direction=item.direction,
                ...
                end_dt=item.end_dt,
                end_value=item.end_value,
                reason="窗口左侧位于持久化锚点之前，尚不能作为正式结构",
                source_indexes=(item.index,),
            )
            for item in result.stable_strokes[:unresolved_count]
        )
        tail_values = tuple(
            ProvisionalLine(
                structure="stroke",
    ...
    values = unresolved_values + tail_values
    _add_provisional_lines(
...
def _add_result_provisional_segments(
    ...
    left_values = tuple(
        ProvisionalLine(
            ...
            end_dt=item.end_dt,
            end_value=item.end_value,
            reason="窗口首段起点无法由当前有限历史证明，仅作为候选",
            source_indexes=tuple(x.index for x in item.strokes),
        )
        for item in result.unresolved_prefix_segments
    )
    tail_values = tuple(
        ProvisionalLine(
            structure="segment",
    ...
    values = left_values + tail_values
    _add_provisional_lines(
...
def _add_strokes(fig: go.Figure, result: AnalysisResult, style: ChartStyle) -> None:
    if not result.resolved_strokes:
        return
    ...
    customdata: list[list[object] | None] = []
    texts: list[str | None] = []
    for stroke in result.resolved_strokes:
        detail = [
            stroke.index,
