...
from .trading_points import build_macd_anchor, detect_trading_points
...
class AnalysisResult:
    ...
    # 已进入正式线段几何范围、只增不减的笔前缀；不包含仅用于确认的尾部证据笔。
    ...
    # 输入窗口第一根 K 之前的 MACD 状态；窗口从真实历史起点开始时为 None。
    macd_anchor: MacdAnchor | None
    # 当前最后一根 K 收盘后的精确 MACD 状态，供滚动窗口持久化。
    macd_final_anchor: MacdAnchor | None
...
class StructureState:
    ...
    def update(self, bar: RawBar) -> None:
        bar_position = self._processed_bar_count
        ...
        self._advance_commits(
            canonical,
            committed_at=bar.close_time,
            committed_at_bar_position=bar_position,
        )
        ...
        *,
        ...
        for i in range(left_skip, commit_stop):
            ...
            evidence = SegmentEvidence(
                ...
                confirmation_available_at=evidence.confirmation_available_at,
                confirmation_stroke_fingerprint=evidence.confirmation_stroke_fingerprint,
                committed_at=committed_at,
                committed_at_bar_position=committed_at_bar_position,
            )
        ...
        # 只封存正式线段实际覆盖到的笔，不再把“确认线段时观察到的证据尾笔”
        # 一并冻结。确认笔及其共享端点仍可能被后续更极端分型迁移；把它写入
        # stable_strokes 会令原始检测链失去旧端点后无法重连，造成正式结构永久停更。
        target_count = max(x.end_position for x in new_evidence) + 1
    ...
    def _compose_canonical_strokes(
        ...
        # 原始检测链若改写了“仅用于确认的证据尾部”，稳定几何前缀保持原值，
        # 并从最后一条正式线段的真实端点重新接入。stable_strokes 只到正式
        # 线段 end_position，因此正常共享端点迁移不应再穿过这里。
...
def _build_analysis_result(
    ...
    *,
    ...
    trading = detect_trading_points(
        ...
        macd_history_anchored=state.left_boundary_anchored,
        macd_anchor=macd_anchor,
    )
    ...
    macd_final_anchor = None
    if bars and (state.left_boundary_anchored or macd_anchor is not None):
        macd_final_anchor = build_macd_anchor(bars, anchor=macd_anchor)
    ...
    return AnalysisResult(
        ...
        macd_anchor=macd_anchor,
        macd_final_anchor=macd_final_anchor,
        metadata=metadata,
    )
...
def _offset_evidence(
    ...
    *,
    ...
    return SegmentEvidence(
        ...
        segment_symbol=item.segment_symbol,
        segment_interval=item.segment_interval,
        segment_fingerprint=item.segment_fingerprint,
        confirmation_available_at=item.confirmation_available_at,
        confirmation_stroke_fingerprint=item.confirmation_stroke_fingerprint,
        committed_at=item.committed_at,
        committed_at_bar_position=item.committed_at_bar_position,
    )
