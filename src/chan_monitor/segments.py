    ...
    *,
    ...
    for i, segment in enumerate(segments):
        ...
        if i < len(evidence_values):
            ...
            if (
                reversible_from is not None
                and item.confirmed_at_position >= reversible_from
            ):
                issues.append(
                    SegmentDiagnostic(
                        code="SEGMENT_CONFIRMATION_USES_REVERSIBLE_LAST_STROKE",
            ...
            if resolved_target is SegmentValidationTarget.COMMITTED:
                if item.committed_at is None or item.committed_at_bar_position is None:
                    ...
                    if 0 <= item.confirmed_at_position < len(values):
                        available_at = _stroke_available_at(values[item.confirmed_at_position])
                        if item.committed_at < available_at:
                            issues.append(
                                SegmentDiagnostic(
...
def detect_segments_from_anchor(
    ...
    *,
    ...
    endpoints = stroke_endpoints(values)
    if len(values) - start_position < 3:
        return SegmentDetectionResult(
            segments=(),
            markers=(endpoints[start_position],),
            candidates=(),
            unfinished_strokes=values[start_position:],
            unresolved_prefix_strokes=(),
            feature_elements=(),
            feature_fractals=(),
            evidence=(),
