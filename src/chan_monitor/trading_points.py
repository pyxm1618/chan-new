...
def detect_trading_points(
    ...
    *,
    ...
    segment_commit_times: Mapping[int, datetime] | None = None,
    ...
    * 三买/三卖：一个已完成线段离开固定线段中枢，紧随其后的第一个反向线段
    ...
    ``segment_commit_times``。缺少任一线段的 committed_at 时，接口安全关闭，
...
def _macd_histogram(
    ...
    *,
    ...
    if not bars:
        return _MacdComputation({}, bool(history_anchored or anchor), anchor)

    if anchor is not None and anchor.asof >= bars[0].close_time:
        raise ValueError("MacdAnchor 必须对应输入窗口第一根 K 之前的已处理状态")
    ...
    return _MacdComputation(histogram, exact, final)
...
def _formal_segment_commit_times(
    ...
    *,
    segment_evidence: Sequence[SegmentEvidence],
    explicit: Mapping[int, datetime] | None,
) -> dict[int, datetime]:
    """汇总正式提交时间；无 committed_at 的结构证据不得升级为买卖点。"""
    result: dict[int, datetime] = {}
    for item in segment_evidence:
        if item.committed_at is not None:
            result[item.segment_index] = item.committed_at
    for index, committed_at in (explicit or {}).items():
        if not isinstance(committed_at, datetime):
            raise TypeError("segment_commit_times 的值必须是 datetime")
        result[int(index)] = committed_at
    # 明确拒绝为不存在的线段偷偷注入提交时间，避免索引错位。
    by_index = {x.index: x for x in segments}
    unknown = set(result) - set(by_index)
    if unknown:
        raise ValueError(f"segment_commit_times 包含未知线段索引：{sorted(unknown)}")
