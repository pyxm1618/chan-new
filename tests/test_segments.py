...
def test_false_actual_break_is_rejected_then_scanning_continues() -> None:
    ...
    assert len(result.unfinished_segment_strokes) == 12
    assert max(
        position
...
def test_5000_bar_feature_sequence_reaches_tail_and_matches_reference() -> None:
    ...
    assert len(result.segments) == 57
    assert len(result.feature_elements) == 659
    assert len(result.feature_fractals) == 88
    assert len(result.unfinished_segment_strokes) == 6
    assert max(
        position
        for element in result.feature_elements
        for position in element.stroke_positions
...
def test_raw_bar_prefix_does_not_confirm_segment_with_reversible_last_stroke() -> None:
    ...
    before = analyze_bars(demo_bars(235, symbol="BTCUSDT", interval="5m"))
    after = analyze_bars(demo_bars(236, symbol="BTCUSDT", interval="5m"))
    ...
    # 235 根时旧实现会额外确认一条依赖第 20 笔的实线段；修复后不再输出。
...
def test_confirmed_segments_are_monotonic_across_raw_bar_prefixes() -> None:
    """必须按原始 K 前缀复算，不能只切最终笔链。"""
    bars = demo_bars(270, symbol="BTCUSDT", interval="5m")
    previous: tuple = ()
    for count in range(220, 271):
        result = analyze_bars(bars[:count])
        current = _signature(result)
        assert previous == current[: len(previous)]
        assert all(
            item.confirmed_at_position < len(result.strokes) - 1
            for item in result.segment_evidence
        )
        previous = current
