...
def test_migrated_confirmation_tail_uses_immutable_evidence_time_snapshot() -> None:
    state, _ = _run(509)
    ...
    assert all(item.confirmation_available_at is not None for item in state.evidence)
    assert all(item.confirmation_stroke_fingerprint for item in state.evidence)
