...
def test_5000_raw_bar_prefixes_have_zero_confirmed_structure_retractions() -> None:
    ...
    assert len(state.stable_strokes) == 328
    assert len(state.resolved_stable_strokes) == 328
    assert len(state.provisional_strokes) == 10
...
def test_analysis_result_keeps_all_stable_and_provisional_strokes_separate() -> None:
    ...
    assert len(result.stable_strokes) == 328
    assert len(result.resolved_strokes) == 328
    assert len(result.provisional_strokes) == 10
